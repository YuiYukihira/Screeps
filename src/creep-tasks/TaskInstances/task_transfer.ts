...
export class TaskTransfer extends Task {
	...
	get target(): transferTargetType { return super.target as transferTargetType; }
	data: {
	...
	constructor(target: transferTargetType,
		...
		this.data = { resourceType: resourceType, amount: amount }; 2
	}
	...
	isValidTarget() {
		...
		if (target instanceof Creep) {
			return _.sum(target.carry.getUsedCapacity) <= target.carryCapacity - amount;
		} else if (isStoreStructure(target)) {
			return _.sum(target.store.getFreeCapacity) <= target.storeCapacity - amount;
