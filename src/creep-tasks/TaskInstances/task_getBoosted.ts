...
function any<T>(collection: ArrayLike<T>, f: (value: T) => boolean): boolean {
...
export class TaskGetBoosted extends Task {
	...
	isValidTask() {
		let lifetime = any(this.creep.body, (part: BodyPartDefinition) => part.type == CLAIM) ? CREEP_CLAIM_LIFE_TIME : CREEP_LIFE_TIME;
